...
/**
 * Class IrisLandlordsReferencingAbstractController
 *
 * @author Simon Paulger <simon.paulger@barbon.com>
 */
abstract class IrisCompleteReferenceAbstractController extends Zend_Controller_Action
{
    /**
     * @var Zend_Config
     */
...
//        // As some templates are borrowed from Connect, look there too
        ...
        if ('submit' != $actionName) {

            // todo: instead use the Zend_Auth mechanism commented out below
            $this->authSession = new Zend_Session_Namespace('homelet_completereference');
...
//            $this->_auth->setStorage(new Zend_Auth_Storage_Session('hl_continuereference'));
            ...
            $exceptionMessage = '';
            try {
                $response = $this->getIrisSystemContext()->getSystemApplicationClient()->validateReviewLink(
            ...
            if (!$exception && 200 == $response->getStatusCode()) {
                ...
                // Show error template
                $this->renderTwigView('/iris-complete-reference/error.html.twig', array(
                    'bodyTitle' => 'Error',
